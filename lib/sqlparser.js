...
SqlParser.parse = function( $query, $replaces ) {
	...
	switch ($query_type) {
		...
		case 'REPLACE':
			...
			break;
		case 'SELECT':
			throw { message: 'SELECT not supported yet', code: 'syntax_error', }
			break;
		case 'DELETE':
			throw { message: 'DELETE not supported yet', code: 'syntax_error', }
			break;
		default:
			throw { message: 'unuspported query', code: 'syntax_error', }
	}
...
SqlParser.split_insert = function($query) {
	...
	if ($q[0] !== 'INTO')
		throw { message: 'You have an error in your SQL syntax, expected INTO', code: 'syntax_error', }
	$q.shift()
...
SqlParser.split_update = function($query) {
	...
	if ($q[0] !== 'SET')
		throw { message: 'You have an error in your SQL syntax, expected SET', code: 'syntax_error', }


	$q.shift()

	if ($q.indexOf('WHERE') === -1 )
		throw { message: 'You have an error in your SQL syntax, expected WHERE', code: 'syntax_error', }
