...
SqlParser.parse = function( $query, $replaces ) {
	...
	switch ($query_type) {
		...
		case 'SELECT':
			...
			//var sqp = sqliteParser($query).statement[0]
			...
			switch (w.op) {
				...
				case 'AND':
					...
					$parsed.WHERE[ and1.left.column ] = {
						...
						value:
							(
								and1.right.type === 'string' ?
								eval(and1.right.string)
								:
								(
									and1.right.type === 'number' ?
									parseInt(and1.right.number)
									:
									undefined // unhandled
								)
					...
					switch ( and2.op ) {
						...
						case '<=':
							...
							$parsed.WHERE[ and2.left.column ] = {
								...
								value:
									(
										and2.right.type === 'string' ?
										eval(and2.right.string)
										:
										(
											and2.right.type === 'number' ?
											parseInt(and2.right.number)
											:
											undefined // unhandled
										)
									),
						...
						case 'BETWEEN':
							...
							$parsed.WHERE[ and2.left.column ] = {
								...
								value: [
									(
										and2.right.left.type === 'string' ?
										eval(and2.right.left.string)
										:
										(
											and2.right.left.type === 'number' ?
											parseInt(and2.right.left.number)
											:
											undefined // unhandled
										)
									),
									(
										and2.right.right.type === 'string' ?
										eval(and2.right.right.string)
										:
										(
											and2.right.right.type === 'number' ?
											parseInt(and2.right.right.number)
											:
											undefined // unhandled
										)
									)
				...
				switch (h.op) {
						...
						$parsed.FILTER[ h.left.column ] = {
							...
							value: 
								(
									...
									(
										h.right.type === 'number' ?
										parseInt(h.right.number)
										:
										h.right.type === 'boolean' ?
										h.right.value:
										undefined // unhandled
									)
								),
					...
					case 'BETWEEN':
						...
						$parsed.FILTER[ h.left.column ] = {
							...
							value: [
								(
									h.right.left.type === 'string' ?
									eval(h.right.left.string)
									:
									(
										h.right.left.type === 'number' ?
										parseInt(h.right.left.number)
										:
										undefined // unhandled
									)
								),
								(
									h.right.right.type === 'string' ?
									eval(h.right.right.string)
									:
									(
										h.right.right.type === 'number' ?
										parseInt(h.right.right.number)
										:
										undefined // unhandled
									)
								)
...
/*
SqlParser.split_select = function ( select ) {

	console.log("split_select", select )
	var fields = []
	SqlParser.split_sql(select, [','] , ["\"","'","`","{","}","[","]","(",")"] ).forEach(function(field) {
