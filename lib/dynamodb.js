...
var dynamo_parser = require('./parser.js');
...
DynamoSQL.prototype.query = function($query, $replaces, cb ) {
	...
	switch (sqp.statement) {
		...
		case 'SELECT':
			...
			if (global.DDBSQL === true )
				console.log("SELECT", JSON.stringify(sqp, null, "\t"))
			...
			$db.RawProjectionExpression(sqp.dynamodb.ProjectionExpression)
			...
			if (sqp.dynamodb.having) {
				...
				switch (h.op) {
					...
					default:
						return $cb( new SyntaxException( '[AWSPILOT] Unhandled operation ' + h.op ) )// there is only one op and is not '='
		...
		case 'SCAN':
			...
			if (global.DDBSQL === true )
				console.log("SCAN", JSON.stringify(sqp, null, "\t"))
			...
			//if (sqp.columns.length > 1 ) {
			//	if (sqp.columns.map(function(c) { return c.type }).indexOf('star') !== -1)
			//		return $cb(  new SyntaxException( "[AWSPILOT] you can not have both * and column names in SCAN" ) )
			//}

			//sqp.columns = sqp.columns.filter(function(c) { return c.type !== 'star'})

			//if (sqp.columns.filter(function(c) { return c.hasOwnProperty('alias') }).length)
			//	return $cb(  new SyntaxException( "[AWSPILOT] 'SCAN field AS alias' not supported, yet!" ) )

			// after removing *, handle remaining fields
			//if (sqp.columns.length) {
			//	$parsed.FIELDS = sqp.columns.map(function(c) { 
			//		return c.column 
			//	})
			//}
